...
Selectors are evaluated against the live registry holders on the server. Modern `tag:c:armors`, exact `id:selling_bin:selling_bin`, and `all:*` selectors work for item-on-block rules, and the legacy `#c:armors` spelling remains supported. Selective item-on-block rules require a nonempty held stack. A whole block `all:*` item-on-block rule also gates an empty hand menu open. A missing stage is denied before the protected interaction continues. The optional bounded support capture is documented in [Interaction locks](docs/troubleshooting/interaction-locks.md).
...
Guide text can use these fixed placeholders. `{stage_name}` is the current stage name, `{stage_requirements}` is its visible prerequisite summary, `{remaining_requirements}` lists unmet trigger or purchase requirements when available, and `{stage_progress}` shows the current trigger progress or `Unavailable`. Unknown placeholders remain visible and are reported by the editor so they can be corrected. Use doubled braces for literal braces, for example `{{stage_name}}` displays `{stage_name}`. Placeholders only insert displayed values. They cannot run commands, grant stages, or change progression.
