...
- **Pack-authored What to do next guide.** Add an optional `[guide]` table with `how_to_unlock`, `next_steps`, `where_to_find`, and `recommendation = "auto"`, `"include"`, or `"exclude"`. The editor presents these as plain language fields with hover help, a placeholder chooser, preview, and character limits. Use `{stage_name}`, `{stage_requirements}`, `{remaining_requirements}`, and `{stage_progress}` for fixed displayed values, or doubled braces for literal braces. Players use the normal advancement style map, then open the selected stage's gold `?` control beside the close button to read its unlock help. The separate guide keybind selects the next suggested stage. Set `general.enable_stage_guide = false` to disable the guide while leaving the normal stage map available.
...
- **EMI + JEI both optional (independent controls)** — locked items and recipes can be filtered in EMI and JEI without making either viewer a hard dependency. `[jei].enabled` and `[emi].enabled` are independent and both default to enabled. Stage grants and revokes resend each player's authoritative viewer snapshot before refreshing the enabled viewer. The recipe-viewer reload path remains safe when EMI is absent.
...
5. Run `/pstages editor` as a permission level 3 operator to open the local React stage editor. It opens directly on the stage list. Use the guided stage workspace, the movable and zoomable Player UI, its read only Player preview, Registry, Settings, or Extensions without touching a file. Direct TOML remains available under Source. Reload at runtime with `/pstages reload`.
...
| `[structures]` | `locked_entry` + `[structures.rules]` (`entry_allowed`, `priority`, `prevent_block_break`, `prevent_block_place`, `prevent_explosions`, `disable_mob_spawning`, **`entry_padding`**) | Attribute entry and protection independently for exact generated structures. `entry_allowed = true` removes only that stage's entry denial, so a protection stage can still deny placement after another stage unlocks entry. |
...
- **`[general]`** — `starting_stages`, `team_mode` (`ftb_teams` / `solo`), `linear_progression`, `reapply_starting_stages_on_login`, `debug_logging`, and `enable_stage_guide`.
