...
Denied direct interactions also resend authoritative inventory state to correct client prediction. Selective restrictions inside an open GUI use a separate `item_into_inventory` rule; `block_right_click` controls access independently. Actual Selling Bin sale and client acceptance remain subject to the [verification record](docs/verification/selling-bin-interaction-repair.md).
