...
- **Pack-authored What to do next guide.** Add an optional `[guide]` table with `how_to_unlock`, `next_steps`, `where_to_find`, and `recommendation = "auto"`, `"include"`, or `"exclude"`. The editor presents these as plain language fields with hover help and character limits. Players switch between the Stages and What to do next tabs in the same advancement style menu, or use the separate guide keybind. Set `general.enable_stage_guide = false` to disable the guide while leaving the normal stage map available.
