...
- **Pack-authored What to do next guide.** Add an optional `[guide]` table with `how_to_unlock`, `next_steps`, `where_to_find`, and `recommendation = "auto"`, `"include"`, or `"exclude"`. The editor presents these as plain language fields with hover help and character limits. Players use the normal advancement style map, then open the selected stage's gold `?` control beside the close button to read its unlock help. The separate guide keybind selects the next suggested stage. Set `general.enable_stage_guide = false` to disable the guide while leaving the normal stage map available.
