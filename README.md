...
- **FTB Quests deep gating** — the `Quest` / `Chapter` `required_stage` field now also gates `TeamData.canStartTasks`, so progression is blocked even if visibility is overridden by another mod. Optional `integration.ftbquests.team_mode` delegates team owned provider operations to FTB Teams' `TeamStagesHelper`. Personal and server stages use ProgressiveStages for checks, grants and removals.
...
| **FTB Library** | `StageProvider` proxy registration for native stage checks, tasks and rewards using the provider route. Explicit FTB team storage settings bypass this provider |
| **FTB Teams** | Default backend when `team_mode = "ftb_teams"`; optional `integration.ftbquests.team_mode` delegates team owned quest provider operations while preserving personal and server ownership |
...
- **`[integration.ftbquests]`** — `enabled`, `team_mode` (delegates team owned provider operations to FTB Teams' `TeamStagesHelper`), `recheck_budget_per_tick`.
