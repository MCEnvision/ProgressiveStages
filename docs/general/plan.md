...
When the guide is enabled, the player opens What to do next through its top tab or dedicated key mapping in the existing advancement style stages screen, sees visible stages whose prerequisites are met, chooses among branches, reads instructions about unlocking and locations, and sees guidance refresh after stage or definition changes. The main config can disable guidance while preserving the normal Stages map. The author workflow is to edit those guide fields in the GUI, review and apply a draft, and verify the same text in game. The delivery workflow ends with the tested 3.1.0 JAR on both established platforms, including the updated web editor served by that JAR.
...
### DEC-007 — Editor help on hover

**Status:** RESOLVED  
**Selected choice:** Keep Stage name, Stage icon and Required stages as plain labels and place short explanations and examples in Minecraft style hover tooltips with equivalent keyboard access.  
**Rationale:** The owner wants discoverable help without permanently crowding the form. Validation and save feedback remain visible.  
**Affected requirements:** CORE-REQ-004, CORE-REQ-006, CORE-REQ-009  
**Supersedes:** none

### DEC-008 — Advancement style guide access and configuration

**Status:** RESOLVED  
**Selected choice:** Preserve the existing advancement style stages screen, add top Stages and What to do next tabs and a dedicated guide key mapping, control guide availability from the main config, and omit pack creator attribution from the GUI.  
**Rationale:** The owner wants familiar navigation and optional guidance. IF-003 fixes the engineering defaults at enabled and an independently rebindable, initially unbound guide key, preserving the existing key convention.  
...
**Acceptance:** CORE-AC-011, CORE-AC-012, CORE-AC-013, CORE-AC-018, CORE-AC-019.
...
Show every qualifying branch in stable ascending `uiSortOrder`, category, display name and stage ID order, using locale independent string comparisons and stage ID as the final tie breaker. Do not choose a forced unique next stage. Label the guide tab What to do next and identify its candidates as prerequisites met, not guaranteed immediately unlockable. Reuse existing slot, cost and trigger information to explain unmet acquisition conditions without replacing author text or bypassing server validation. Selecting a candidate focuses its existing node and inspector in the guide tab. When the guide is enabled, a normally visible selected stage may show its guide even when it is not recommended or already owned. No guide, search result, cross reference, category summary, aggregate count or empty state may disclose a hidden or unrevealed stage through presentation. This retains the existing visibility policy and does not claim that the existing full definition sync provides cryptographic secrecy from a modified client. Empty guidance means no invented instructions; an empty recommendation list says no suggested stages are available, not that all content is complete.

### Guide navigation and main configuration

Preserve the existing advancement style in `StageTreeScreen`: Minecraft window texture, item nodes, connected stage layout, backgrounds, font, pan and zoom, category controls, search and selected-stage inspector. Add two top tabs, Stages and What to do next, within that same screen. Keep the current map in Stages. The guide tab reuses the connected item-node map and inspector, emphasizes eligible next stages and retains only normally visible prerequisite context. Selecting a suggested node shows How to unlock, Next steps and Where to find in the existing inspector with wrapped, scrollable text. Returning to Stages focuses that same visible node. Preserve map position, selection, search and category state across tab changes; clamp invalid selection after accepted state changes. Fit tabs without covering search or category controls, including compact GUI scales. Use Minecraft controls, focus cues and hover tooltips, not a separate dashboard appearance. Both tabs support pointer and keyboard navigation with a clear selected state. Render no pack creator attribution, author-source footer or equivalent explanatory banner in either GUI or its preview. The guide text itself remains literal authored content.
...
The accepted server guide-enabled Boolean travels with the protocol 3 accepted definition snapshot to both display consumers. Main config loading or reloading on the server schedules publication through the same revision owner, even when guide text has not changed. Apply policy and definitions together, clear the cached policy on disconnect, and never infer server policy from the local COMMON config. Loading remains unavailable until the current complete snapshot arrives. Maintain the existing separation of explicit opening from refresh in `ClientTriggerProgress`, `OpenStageGuiPayload` and `StageTreeScreen`; an asynchronous response cannot reopen a closed screen, reset the active tab or override a newer disabled policy. No client request changes the server setting or bypasses existing stage visibility and purchase permissions.
...
2. Documentation records hover/focus help, top tabs, guide key assignment, main config key/default/reload/disabled behavior, protocol 3 matching client/server requirements, empty guide behavior, hidden/reveal limits and release support collection. Wiki publishes only merged behavior.
...
4. CORE-AC-018: The actual screen retains its advancement style map and inspector, supports top Stages and What to do next tabs, and opens the guide directly through its independent Controls mapping. Original entry paths still open Stages. Tab switching, retained map state, typing, repeated or conflicting key presses and late responses cause no accidental opening, tab reset or gameplay action. No pack creator attribution appears.
5. CORE-AC-019: `general.enable_stage_guide` defaults true, is validated in the main config and is server authoritative on login and reload. False removes guide UI and makes its key a no-op, including while open; ordinary stages access and saved guide data remain intact. Unknown policy, reconnect between differently configured servers, invalid reload and reenable follow IF-003 and IF-004.
...
| `guide_enabled:boolean`, `guide_policy:enum`, `candidate_count:int`, `visible_candidate_ids:list`, `guide_lengths:list<int>`, `sync_revision:long`, `reason:enum` | Phase 002 progression extension from accepted server policy and after visibility filtering; code point counts and IDs | Accepted guide policy explains enabled or disabled output; auto/exclude/owned/prerequisite reasons match visible state; counts and summaries include only visible candidates and never disclose hidden totals. Server records never claim a client tab or rendered result |
