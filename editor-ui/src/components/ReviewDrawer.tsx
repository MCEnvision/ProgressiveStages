...
export function ReviewDrawer() {
  ...
  return <div className="review-backdrop" onMouseDown={event => { if (event.target === event.currentTarget) closeReview(); }}><section className="review-drawer" role="dialog" aria-modal="true" aria-label="Apply stage changes">
    ...
    {applyResult ? <div className="apply-result"><Icon name="check"/><div><strong>{applyResult.restartRequired ? "Saved, restart required for some settings" : "The live server is synchronized"}</strong><p>{applyResult.explanation || `Configuration revision ${applyResult.configurationRevision}.`}</p>{applyResult.restartRequired ? <small>Pending restart settings: {applyResult.pendingRestartSettings?.join(", ")}</small> : null}<code>{applyResult.transactionId}</code></div></div> : null}
