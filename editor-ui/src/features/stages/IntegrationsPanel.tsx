...
export function IntegrationsPanel({ stage }: { stage: StagePackage }) {
  ...
  const editInteraction = (row?: InteractionModel, index = -1) => openDialog({ title: row ? "Edit interaction" : "Add interaction", description: "Build a server checked item and target interaction.", content: <InteractionForm row={row} onCancel={closeDialog} onSave={async (next, pairInventory) => { const rows = interactions.map(entry => entry.sourceText || serializeInteraction(entry)); if (index < 0) await mutateFile(stage.rulesPath, appendRow(rulesText, pairInventory ? serializeContainerInsertionPair(next) : serializeInteraction(next)), pairInventory ? "Item and inventory insertion rules saved" : "Interaction saved"); else { rows[index] = updateInteractionBlock(rows[index], next); await mutateFile(stage.rulesPath, replaceInteractions(rulesText, rows), "Interaction saved"); } closeDialog(); }} />, width: "wide" });
