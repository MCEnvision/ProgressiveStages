...
This phase repairs the entity presence performance defect tracked by `CORE-REQ-002` while preserving the existing server-authoritative multiplayer behavior for entity visibility, spawning, targeting, interaction, and lifecycle. It exists because issue `#24` identifies condition-context construction in the entity tracking path as a server-thread hotspot. The measurable outcome is an immutable context boundary keyed by player, server tick, and authoritative state revision. That boundary eliminates full context reconstruction for each tracked entity, invalidates stale same-tick state before the next decision, and satisfies the approved `DEC-004` correctness and performance gate.
...
| VERIFIED | Performance decision | `DEC-004` selects exactly one immutable context for each player, server tick, and authoritative state revision tuple, requires same-tick revision invalidation, forbids per-entity rebuilds within a valid tuple, and sets the sampled work and p95 MSPT thresholds | `DEC-004` in `plan.md` §9 | Invalid only by a new locked owner decision |
...
- `DEC-004` remains resolved with the selected choice, “Use one immutable context per player, server tick, and authoritative state revision, with same-tick revision invalidation and no per-entity rebuild, under-five-percent sampled work, and at-most-ten-percent p95 MSPT regression thresholds.”
...
- CORE-REQ-002 constructs exactly one immutable condition context for each valid player, server tick, and authoritative state revision tuple and reuses it for every tracked-entity decision in that tuple.
- CORE-REQ-002 treats the authoritative state revision as covering the active compiled rule revision and every player-relevant stage, team, score, and metric fact consumed by the context. A same-tick change to any of those facts invalidates the old tuple before the next decision; the new tuple constructs at most one replacement context and subsequent decisions reuse it.
- CORE-REQ-002 prohibits a tracked entity from triggering a full condition-context rebuild while the player, server tick, and authoritative state revision tuple remains valid.
...
2. `P001-TASK-002` executes `CORE-REQ-002` by defining the immutable player, server-tick, and authoritative-state-revision tuple, ownership, lifetime, player isolation, and observable construction-count contract.
...
- Construction-count proof that each player, server tick, and authoritative state revision tuple receives exactly one immutable condition context while multiple tracked entities are evaluated, and that no entity evaluation creates a second context for the same tuple.
...
- Proof of the exact approved gate: exactly one immutable condition context per player, server tick, and authoritative state revision tuple; same-tick authoritative changes invalidate the old tuple before the next decision and allow at most one context for the new tuple; no full context rebuild occurs per tracked entity; under the controlled fixture, entity presence work stays below five percent of server thread time and adds at most ten percent p95 MSPT versus presence rules disabled.
...
- Exactly one immutable condition context exists for each player, server tick, and authoritative state revision tuple that is evaluated. Same-tick rule, stage, team, score, and metric revisions invalidate the old tuple before the next decision, the new tuple constructs at most one replacement context, and no full context rebuild occurs per tracked entity while a tuple remains valid.
...
|---|---|---|---|---|
| Player-tick-revision condition-context boundary | CORE-PHASE-004 | Context is immutable and scoped to exactly one player, server tick, and authoritative state revision tuple. Same-tick authoritative changes invalidate the prior tuple before the next decision, and no tracked entity rebuilds a context while its tuple remains valid | Transient only; no persisted stage, configuration, command, API, or network schema change | Tuple construction-count, isolation, same-tick revision, and invalidation tests |
...
| P001-TASK-002 | CORE-REQ-002 | Establish immutable player, server-tick, and authoritative-state-revision tuple identity and construction-count observability | P001-TASK-001, master architecture | Context ownership contract and bounded observability | MinecraftConditionContextFactory, EntityPresenceEnforcer | Multiple entity evaluations construct exactly one context for each evaluated tuple and never reuse it across players, ticks, or revisions |
| P001-TASK-003 | CORE-REQ-002 | Implement authoritative invalidation and teardown boundaries | P001-TASK-002 | Revision-aware invalidation behavior and stale-state prevention | EntityPresenceEnforcer, CompiledRuleEngine, player-relevant server state interfaces | Rule, stage, team, score, and metric changes between same-tick decisions invalidate the old tuple before the next decision and construct at most one context for the new tuple; transition, disconnect, stop, and reload tests force the next valid context |
...
The context boundary is player-scoped and server-thread confined. Its identity is the tuple of player identity, server tick, and authoritative state revision. The authoritative state revision represents the compiled rule revision and every stage, team, score, and metric fact consumed by the context. Reuse is legal only when every tuple value matches. Exactly one immutable context may be constructed for an evaluated tuple, regardless of how many tracked entities require decisions. A context must not be shared between players, levels, server ticks, or authoritative state revisions. It is transient and must not enter saved data, configuration, packets, or public APIs. Construction-count instrumentation must be bounded to tests or diagnostics and must not add broad allocation, filesystem access, blocking work, full collection scans, or per-tick log spam.

Invalidation must cover every authoritative source named by the master architecture: rule reload, stage mutation, team mutation, score or metric mutation, dimension transition, session transition, disconnect, and server stop. A rule, stage, team, score, or metric mutation during a server tick must advance the authoritative state revision, make the old tuple unavailable before the next entity-presence decision, and permit at most one context to be constructed for the new tuple. Invalidation may eagerly discard a context or advance its revision, but it must make stale reuse impossible. Concurrent or reentrant server events must never expose a partially built context or permit two contexts for the same tuple. Failure to build a valid context must follow existing safe rule behavior and emit actionable bounded diagnostics; it must not silently share another player's state or retain a failed context as the tuple value.
...
|---|---|---|---|---|
| Context reused for another player | Player identity mismatch or isolation assertion | Reject reuse and construct the correct player's context for that player's current tick and authoritative state revision tuple | Correct cache key or ownership boundary and rerun all correctness and profile tests | Two-player same-entity isolation test |
| Context reused after a same-tick rule, stage, team, score, or metric mutation | Revision mismatch, stale-decision assertion, or old tuple access after mutation | Advance the authoritative state revision, invalidate the old tuple before the next decision, construct at most one context for the new tuple, and evaluate from current server state | Fix the missing mutation hook or revision advancement and rerun all dependent correctness and profile tests | Parameterized same-tick invalidation suite with two or more post-mutation decisions |
...
| P001-TASK-002 | Assert context immutability, player identity, tick identity, authoritative state revision identity, and construction count | Evaluate many entities for one player in one unchanged tuple | Diagnostic count is exactly one for that tuple in controlled runtime | Attempt cross-player, cross-tick, cross-level, and cross-revision reuse | Focused unit results and bounded diagnostic output |
| P001-TASK-003 | Parameterized revision and invalidation tests | Mutate rule, stage, team, score, and metric state between decisions in the same tick, then exercise all transition and teardown boundaries | Same-tick mutation, reload, dimension, session, disconnect, and stop workflows | Prove the old tuple is unavailable before the next decision and the new tuple constructs no more than one context | Same-tick and lifecycle invalidation test report |
...
| Duplicate contexts exist for a new same-tick revision tuple | Construction count exceeds one for the new tuple after invalidation | Enforce one atomic or server-thread-confined construction path per tuple | Correct tuple creation and rerun from the first mutation test | Construction-count, multiplayer, and profile results | Rerun tasks 002 through 008 |
