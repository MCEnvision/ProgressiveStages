...
This phase proves that the production editor, in-game progression map, and shipped artifact deliver the user-visible behavior already promised for ProgressiveStages 3.0.3, including the issue `#25` recipe-lock authoring contract promoted for 3.0.4. It owns CORE-REQ-005, CORE-REQ-006, CORE-REQ-007, and CORE-REQ-011 only. Its measurable outcome is a packaged candidate whose Easy Builder can round trip every supported enchantment restriction, whose classic visual builder persists recipe-output locks through exact `[recipes].locked_items` semantics without rule loss, whose category overlay remains above stage icons without breaking map interaction, and whose shipped resources contain every other previously advertised 3.0.3 capability that the CORE-PHASE-000 audit proved missing.
...
|---|---|---|---|---|
| VERIFIED | Requirement boundary | CORE-REQ-005, CORE-REQ-006, CORE-REQ-007, and CORE-REQ-011 define the complete canonical scope of this phase | `plan.md` sections 6, 12, and 13 | Invalidated by an owner-approved master plan revision |
...
| OBSERVED | Easy Builder recipe locks | The classic visual builder maps a recipe-output lock to generic `[recipes].locked`, although parser and compiler contracts distinguish canonical `[recipes].locked_items` from recipe-identifier `[recipes].locked_ids`; issue `#25` reports that the ineffective rule appears to delete itself | SRC-008, CORE-REQ-011, and the recipe-lock current-state row in `plan.md` | Replaced by a CORE-PHASE-000 reproduction tied to the exact editor bundle, persisted file, and runtime artifact identity |
...
- CORE-REQ-011: make the classic visual builder serialize recipe-output restrictions only as `[recipes].locked_items`, while preserving `[recipes].locked_ids` as the separate recipe-identifier field and refusing to persist generic `[recipes].locked` as a successful recipe lock.
...
- FUT-003 is excluded because requests outside the locked six-issue baseline are novel scope, not a 3.0.3 artifact correction.
...
**Objective:** Produce a candidate artifact that passes production-bundle Easy Builder enchantment and recipe-output-lock round trips, category-overlay render and input tests, and runtime proof for every CORE-PHASE-000 row classified as a missing advertised 3.0.3 capability
**Owner:** EasyBuilder
**Dependencies:** CORE-PHASE-000
**Canonical requirements:** CORE-REQ-005, CORE-REQ-006, CORE-REQ-007, CORE-REQ-011
**Documentation and release impact:** Update `README.md`, `DOCUMENTATION.md`, the documentation index, and affected references only for behavior verified in the packaged candidate; document the canonical recipe-output and recipe-identifier fields, validation, recovery, reload, and runtime behavior; provide artifact-parity, recipe-lock, and client evidence to CORE-PHASE-004 and the 3.0.4 release packet
**Next transition:** CORE-PHASE-004

**Entry criteria**

- CORE-PHASE-000 is integrated and its completion packet identifies the exact source revision, shipped artifact identity, test settings, expected behavior, observed behavior, and assigned requirement for issues `#11`, `#16`, and `#25`.
...
- CORE-REQ-005, CORE-REQ-006, CORE-REQ-007, and CORE-REQ-011 permit shared changes only where the canonical editor data model or client rendering boundary requires them. A shared change must retain server authority, client-only class isolation, packet validation, and existing serialized identifiers.

**Execution order**

1. `P003-TASK-001` converts the CORE-PHASE-000 rows for CORE-REQ-005, CORE-REQ-006, CORE-REQ-007, and CORE-REQ-011 into frozen reproduction fixtures and an allowed-correction ledger.
...
6. `P003-TASK-006` assembles the CORE-REQ-005, CORE-REQ-006, and CORE-REQ-007 integrated candidate evidence, updates verified documentation, inspects the candidate artifact, and runs the combined editor and client regression pass.
7. `P003-TASK-007` executes CORE-REQ-011 by correcting and proving the classic visual builder recipe-lock round trip, then refreshes the combined phase packet against the final integrated candidate.
...
- Production-bundle browser evidence that a recipe-output lock creates, edits, removes, saves, reopens, reviews, and applies as exact `[recipes].locked_items`; a recipe-identifier lock remains `[recipes].locked_ids`; and generic `[recipes].locked` is never accepted or persisted as successful output.
...
| Verified recipe-lock round trip | CORE-PHASE-004 | Visual recipe-output locks persist only as `[recipes].locked_items`, recipe identifiers remain `[recipes].locked_ids`, and the same normalized rules survive reload, synchronize, and enforce | Existing stage schema and runtime semantics remain compatible; ambiguous generic fields never overwrite valid state | Browser workflow, file diff, parser and compiler comparison, reload and sync records, denied and eligible player runtime evidence |
...
|---|---|---|---|---|---|---|
| P003-TASK-001 | CORE-REQ-005, CORE-REQ-006, CORE-REQ-007, CORE-REQ-011 | Freeze reproduction fixtures for issues `#11`, `#16`, and `#25`, select allowed parity corrections, and reject rows that lack sufficient contract or artifact proof | CORE-PHASE-000 completion packet and claim-to-artifact matrix | Immutable phase fixture set and allowed-correction ledger | Issue evidence, editor and recipe-lock workflow contracts, stage map workflow contract, artifact parity matrix | Independent row review confirms every action has a canonical requirement and sufficient source evidence |
...
| P003-TASK-006 | CORE-REQ-005, CORE-REQ-006, CORE-REQ-007 | Run combined verification, update only evidence-backed documentation, inspect the candidate artifact, and assemble integrated candidate evidence for the final recipe-lock pass | P003-TASK-003 through P003-TASK-005 | Integrated candidate evidence consumed by P003-TASK-007 | Build output, JAR resources, `README.md`, `DOCUMENTATION.md`, documentation index, affected references | Clean build, archive listing, documentation comparison, clean diff review, combined browser and client smoke |
| P003-TASK-007 | CORE-REQ-011 | Correct the classic visual builder recipe-lock serializer and prove create, edit, remove, save, reopen, source view, review, apply, persisted reload, client sync, parser and compiler normalization, and runtime enforcement | P003-TASK-001 fixture, recipe-lock schema contract, server authority contract, and P003-TASK-006 candidate evidence | Canonical recipe-output round trip, distinct recipe-identifier behavior, atomic migration or rejection, and refreshed phase completion packet | Classic visual builder, recipe rule serializer, TOML source view, draft validator, parser, compiler, persisted stage files, client synchronization, runtime recipe enforcement | Browser and file round trip plus denied and eligible player runtime tests, malformed or ambiguous input recovery, clean build, candidate JAR inspection, and final combined regression |

P003-TASK-001 must complete before any implementation because it is the only authority for artifact-parity scope. P003-TASK-002 must precede P003-TASK-003 so bundle proof tests the corrected canonical form. After P003-TASK-001, P003-TASK-004 may proceed independently of P003-TASK-002 and P003-TASK-003 because it owns a separate client boundary. P003-TASK-005 corrections may proceed independently of each other only when their audit rows identify disjoint components; shared schema, editor, packet, or client surfaces require serialized integration and rerunning every affected fixture. P003-TASK-006 establishes the integrated candidate evidence for the original phase scope. P003-TASK-007 then completes the recipe-lock correction against that candidate and reruns every editor, schema, synchronization, runtime, documentation, build, and artifact check invalidated by its changes before producing the final packet.
...
Run order is static and unit checks, editor or browser integration, parser and compiler normalization, persisted-file reload and client synchronization, paired-player recipe runtime, client render and input checks, applicable formatter and analysis, clean `./gradlew build`, production-bundle smoke, client runtime smoke, and final JAR inspection. If a failure changes source or generated bundle output, restart at the earliest affected check and always repeat clean build, production-bundle smoke, recipe runtime, client smoke, and artifact inspection. Lower-fidelity source or unit evidence cannot replace browser execution against packaged resources, authoritative file reload and runtime recipe enforcement, or the real client map workflow.
...
- Add 3.0.4 release-note inputs for issues `#11`, `#16`, and `#25`, and each proven parity correction. Release wording remains provisional until CORE-PHASE-005 validates the integrated artifact.
...
- The integrated phase commit identity and a focused diff that contains only CORE-REQ-005, CORE-REQ-006, CORE-REQ-007, CORE-REQ-011, required tests, and evidence-backed documentation.
...
- Negative evidence that malformed, ambiguous, generic-field, or unauthorized editor apply does not mutate server state, overlay clicks do not reach covered nodes, and unpromised features did not enter the diff.
