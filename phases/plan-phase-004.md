...
This phase proves that the integrated outputs of CORE-PHASE-001, CORE-PHASE-002, and CORE-PHASE-003 preserve ProgressiveStages compatibility and regression safety before release integration begins. Its sole canonical requirement is CORE-REQ-008. The phase owns the compatibility matrix, security and authority checks, runtime exercises, artifact inspection, failure routing, recovery rehearsal, and completion evidence for the supported ProgressiveStages 3.0.4 patch boundary. That matrix includes the integrated CORE-REQ-011 recipe-lock result delivered by CORE-PHASE-003 without reassigning ownership of the editor correction.
...
- CORE-REQ-002 through CORE-REQ-007 and CORE-REQ-011 implementation is owned by earlier phases. This phase verifies their integrated result but does not repeat or independently redesign those fixes.
...
2. `P004-TASK-002` validates CORE-REQ-008 configuration, stage schema, persisted stage data, commands, public API compatibility, and valid legacy recipe-lock fixtures without rewriting accepted input or collapsing `[recipes].locked_items` and `[recipes].locked_ids`.
...
6. `P004-TASK-006` verifies CORE-REQ-008 operator authority, malformed input rejection, packet validation, client-server boundary safety, optional classloading isolation, sanitized diagnostics, and atomic preservation when an invalid or ambiguous `[recipes].locked` editor draft is rejected or an unambiguous legacy draft is migrated.
...
8. `P004-TASK-008` inspects the CORE-REQ-008 final JAR, executes client and server smoke workflows from that JAR, proves the packaged Easy Builder visual-form-to-reload-to-runtime recipe-lock workflow, confirms artifact and documentation parity, and rehearses rollback to the last accepted candidate.
...
| Editor, UI, recipe-lock, and artifact parity output | CORE-PHASE-003 | Production editor bundle, client UI, CORE-REQ-011 canonical recipe-lock transaction, and advertised-capability corrections are merged and packaged | Verify completion packet, `[recipes].locked_items` and `[recipes].locked_ids` distinction, preservation evidence, and packaged-resource identity | Stop; route the gap to CORE-PHASE-003 and invalidate editor, schema, runtime, client, and JAR evidence |
...
| P004-TASK-002 | CORE-REQ-008 | Load and exercise supported existing configuration, stage schema, saved stage ownership, commands, public API calls, and valid legacy recipe-lock files | P004-TASK-001, CORE-PHASE-003 completion packet, and existing supported fixtures | Compatibility results, canonical-field distinction, and before-and-after data comparisons | ConfigurationSchema, persistence, command surface, public APIs, stage parser, rule compiler | Normalized behavior assertions, `[recipes].locked_items` versus `[recipes].locked_ids` distinction, no-loss checks, and unsupported-input negative cases |
...
| P004-TASK-006 | CORE-REQ-008 | Review and test trust boundaries, validation, diagnostics, optional isolation, and recipe-draft atomicity | P004-TASK-002 through P004-TASK-005 and CORE-REQ-011 upstream evidence | Security, preservation, migration, and negative-test record | Editor apply, client payloads, permissions, schema validation, persistence transaction, logging | Non-operator denial, invalid and ambiguous `[recipes].locked` rejection, unambiguous migration, unchanged last-valid file and compiled snapshot, bounds checks, absent-mod startup, sanitized logs |
...
| P004-TASK-008 | CORE-REQ-008 | Inspect and smoke-test the packaged JAR, exercise the production Easy Builder recipe-lock transaction, compare documentation, and rehearse candidate rollback | P004-TASK-007 | JAR inventory, smoke evidence, visual-form-to-runtime recipe packet, parity record, recovery result | Packaged editor bundle, metadata and resources, stage files, compiler, recipe enforcement, client, dedicated server, documentation | JAR identity check, visual form to TOML to apply to reload to denied and eligible runtime behavior, editor reopen, documentation comparison, rollback rehearsal |
| P004-TASK-009 | CORE-REQ-008 | Resolve evidence invalidation, confirm no open mandatory finding across the six-issue candidate, and assemble the downstream packet | P004-TASK-001 through P004-TASK-008 | Signed-off phase completion packet for CORE-PHASE-005 | Evidence ledger, six-issue traceability, and release-candidate handoff | Traceability audit confirms every matrix row, issue including #25, artifact, and rerun belongs to one final candidate |
