...
|---|---|---|---|---|
| VERIFIED | Upstream product verification | CORE-PHASE-004 must provide the complete six-issue compatibility packet, including CORE-REQ-011 visual-form serialization, canonical persisted field, reload, compiler, preservation, rollback, and runtime enforcement results, plus security, build, server, client, multiplayer, and final-JAR evidence for the candidate source | CORE-PHASE-004 completion packet | Any source, editor bundle, recipe-lock schema or runtime path, resource, dependency, workflow caller, or release metadata change after the recorded verification invalidates the packet |
...
- Bind the candidate identity to the accepted CORE-PHASE-004 six-issue compatibility packet, including the CORE-REQ-011 results, without reopening or implementing any recipe-editor work in this phase.
...
- CORE-PHASE-004 is merged through its required integration path, `origin/master` contains that merge, and its six-issue completion packet passes without an unresolved mandatory defect. That packet explicitly contains the CORE-REQ-011 canonical-field, visual-form-to-runtime, preservation, rollback, reload, and packaged-JAR results.
...
- CORE-REQ-009 stores a sanitized completion packet containing identities, the accepted six-issue compatibility-packet identity and CORE-REQ-011 result references, commands or workflow inputs, run references, expected and observed results, and evidence-retention locations for CORE-PHASE-006. This is release-evidence binding only and does not absorb CORE-REQ-011 implementation ownership.
...
- An equality audit showing the six-issue compatibility packet, including the CORE-REQ-011 results, and every release evidence record resolve to the same source commit and exact candidate bytes, plus a sanitized runbook for reproducing validation.
...
|---|---|---|---|---|
| CORE-PHASE-004 six-issue completion packet | CORE-PHASE-004 | All compatibility, security, runtime, build, and JAR inspection gates pass for the source entering release integration, including CORE-REQ-011 canonical serialization, persistence, reload, enforcement, preservation, and rollback evidence | Reconcile packet commit, CORE-REQ-011 result references, served editor bundle, and artifact references with updated `origin/master` | Stop; route the defect to its owning prior phase and invalidate candidate work without implementing the recipe-editor correction here |
...
| Verified 3.0.4 candidate | CORE-PHASE-006 | Candidate bytes derive from the signed verified `master` commit, are bound to the accepted six-issue compatibility packet including CORE-REQ-011 results, and passed all Phase 005 validation | Candidate is immutable by identity; any byte, metadata, name, source revision, editor-bundle identity, or compatibility-packet change requires complete revalidation | Candidate identity record, six-issue packet identity, CORE-REQ-011 result references, hashes, source manifest, JAR inventory, and successful workflow run |
...
|---|---|---|---|---|---|---|
| P005-TASK-001 | CORE-REQ-009 | Admit upstream evidence and freeze exact input identities | CORE-PHASE-004, EXT-002, DEC-005 | Admission record containing source commit, six-issue phase packet with CORE-REQ-011 result references, shared revision, and blocker state | Phase completion packets, shared workflow revision, repository ancestry | All input identities exist, are immutable, include the complete CORE-REQ-011 result chain, and agree; otherwise phase remains blocked |
...
The release identity is a closed set containing the signed source commit, exact candidate bytes, filename, size, accepted six-issue compatibility-packet identity, CORE-REQ-011 result references, served editor-bundle identity, SHA-256, SHA-512, source manifest, JAR inventory, SPDX SBOM, build-provenance attestation, SBOM attestation, shared-workflow revision, and verification-run identity. The source commit and artifact hashes are the primary join keys. A rebuild is a new candidate even when its version string is unchanged. No evidence may be reused after a byte, source-identity, editor-bundle, or compatibility-packet change.
...
| Upstream source or six-issue evidence changes during integration | `origin/master`, candidate source commit, served editor bundle, compatibility-packet identity, or CORE-REQ-011 result references differ from the admitted CORE-PHASE-004 identity | Invalidate upstream compatibility, build, and artifact evidence | Reconcile the new revision, rerun affected Phase 004 gates and the CORE-REQ-011 proof chain in its owning phase when affected, then restart candidate generation | Updated six-issue compatibility packet and ancestry record |
...
|---|---|---|---|---|---|
| P005-TASK-001 | Identity-format and dependency-completeness checks | Reconcile the Phase 004 six-issue packet, CORE-REQ-011 result references, served editor bundle, `master`, and EXT-002 identities | Fetch the exact shared revision and verify merge accessibility | Missing, mutable, incomplete, or mismatched input blocks entry | Admission record |
...
- CORE-PHASE-004 six-issue completion packet identity, including CORE-REQ-011 canonical serialization, persistence, reload, enforcement, preservation, rollback, packaged editor-bundle, and runtime results, plus proof that the Phase 005 branch began from the approved updated `origin/master`.
...
- Cross-record identity-equality report binding the six-issue packet and CORE-REQ-011 result references to the exact candidate source and bytes, evidence-retention index, secret scan, release-validation documentation diff, rerun and recovery runbook, and no-publication audit.
...
After GitHub confirms the Phase 005 integration is merged, required checks remain successful, `origin/master` contains the signed resulting commit, and the complete Phase 005 packet passes its identity audit against the accepted six-issue compatibility packet and CORE-REQ-011 results, reread `phases/plan-phase-006.md` through EOF. Begin CORE-PHASE-006 only with the exact validated candidate and completion packet. CORE-PHASE-006 must independently obtain the scope-bound EXT-003 confirmation before publication. If any candidate byte, source revision, editor-bundle identity, six-issue packet, CORE-REQ-011 result reference, workflow pin, release metadata, checksum, SBOM, manifest, or attestation changes, do not transition; return to the earliest invalidated Phase 005 task and regenerate every downstream proof.
