...
Starting branch: envy/3.0.4-phase-003
Starting commit: e1a571ad3bc206ee61e60b12c4b448c2b3c79981
Remote ref: origin/envy/3.0.4-phase-003
Remote commit: e1a571ad3bc206ee61e60b12c4b448c2b3c79981
Observed plan-authoring branch: envy/3.0.4-phase-003
Observed plan-authoring commit: e1a571ad3bc206ee61e60b12c4b448c2b3c79981
...
Authoritative default ref observed during this repair: origin/master
Authoritative default commit observed during this repair: 1cc3d5e6d7227a147b2096c8b85253b083b57a2a
Observed active phase ref: origin/envy/3.0.4-phase-003
Observed active phase commit: e1a571ad3bc206ee61e60b12c4b448c2b3c79981
Execution baseline rule: after a fresh fetch, each phase starts from the then-current verified origin/master that contains and is tagged for its required predecessor; observations in this block are evidence, not permanent branch pins
...
| Current authoritative default | VERIFIED | `origin/master` resolves to `1cc3d5e6d7227a147b2096c8b85253b083b57a2a`, the Phase 002 merge commit, and the signed annotated tag `3.0.4-phase-002` resolves to that commit | Read-only ref, ancestry, and signed-tag inspection during this repair |
...
| Phase 001 completion | VERIFIED | The entity-presence repair completed through merge commit `09b18ad5b91a8c5b59faf1d35821f5c786427b80` on `master`, with signed annotated tag `3.0.4-phase-001` | Phase 001 merge record, signed tag verification, focused tests, dedicated-server fixture, and artifact inspection |
| Phase 002 completion | VERIFIED | The optional-integration work completed through merge commit `1cc3d5e6d7227a147b2096c8b85253b083b57a2a` on `master`, with signed annotated tag `3.0.4-phase-002` | Phase 002 merge ancestry and signed-tag inspection during this repair |
| Phase 003 execution | OBSERVED | `envy/3.0.4-phase-003` and its remote-tracking ref resolve to `e1a571ad3bc206ee61e60b12c4b448c2b3c79981`; that commit descends from the current verified `origin/master` Phase 002 merge | Branch, merge-base, and ref inspection during this repair; task-level completion still requires the Phase 003 evidence contract |
| Curios integration | VERIFIED | At Phase 002 merge `1cc3d5e6d7227a147b2096c8b85253b083b57a2a`, Curios 9.5.1 resolved through the optional adapter boundary and had present, absent, lifecycle, and inventory-conservation evidence; Phase 004 still owns integrated-candidate revalidation | Phase 002 merge commit `1cc3d5e6d7227a147b2096c8b85253b083b57a2a`, signed tag `3.0.4-phase-002`, and optional-integration artifact record |
| Recipe viewers | VERIFIED | At Phase 002 merge `1cc3d5e6d7227a147b2096c8b85253b083b57a2a`, independent default-enabled JEI and EMI controls had individual, combined, disabled, and absent runtime evidence; Phase 004 still owns integrated-candidate revalidation | Phase 002 merge commit `1cc3d5e6d7227a147b2096c8b85253b083b57a2a`, signed tag `3.0.4-phase-002`, configuration tests, and optional-integration artifact record |
...
| Easy Builder recipe locks | OBSERVED | The completed Phase 000 audit recorded the shipped editor's noncanonical generic `[[rules]]` recipe target and preserved the distinct canonical `[recipes].locked_ids` and `[recipes].locked_items` contracts. Phase 003 owns corrected production-bundle, persistence, reload, denied-and-eligible runtime, and rollback proof. | SRC-008, the Phase 000 evidence packet, and CORE-REQ-011 |
| Inventory insertion locks | OBSERVED | The completed Phase 000 audit froze the legacy single-selector interaction and menu seams. Phase 003 now owns the additive paired-selector schema, destination resolution, whole-transaction enforcement, editor workflow, and required runtime evidence. No current source edit is acceptance evidence by itself. | Phase 000 seam map, CORE-REQ-012, and the active Phase 003 branch |
...
| Artifact parity | OBSERVED | The completed Phase 000 matrix found no independently proven missing advertised capability beyond the owned report rows, while Phase 003 still must prove the applicable installed-candidate workflows | Phase 000 claim matrix and CORE-REQ-007 |
...
### Canonical Execution Baseline, Dependency, and Cache Precedence

The immutable saved goal records the creation checkout `5b3077764907249b3711886cca538794f6139acf` and the original Phase 000 entry action as provenance only. It never reopens a completed phase, selects a live implementation branch, freezes `origin/master`, or requires a later phase to recreate the creation checkout. The current master plan, registered phase sequence, and fresh repository evidence own live phase selection. Historical phase blueprints retain their original task wording as an execution record and are not independent status authorities.

Before creating or resuming any phase branch, fetch `origin`, prove that the remote is the intended repository, resolve the current `origin/master`, and verify the immediate predecessor's merged commit and signed annotated phase tag when a predecessor exists. A new phase branch starts from that verified current `origin/master`. An existing applicable phase branch may resume only when its ancestry contains the verified predecessor baseline and current remote reconciliation shows no unintegrated earlier phase. No saved-goal checkout, plan-authoring checkout, historical audit commit, evidence artifact, runtime cache, or hard-coded stale revision may replace this check.
...
- `CORE-PHASE-001` is completed through `09b18ad5b91a8c5b59faf1d35821f5c786427b80` and signed tag `3.0.4-phase-001`. Its player-tick context snapshot is transient runtime state inside the completed entity-presence implementation. It is not a build cache, optional-adapter cache, artifact input, or mutable handoff consumed by Phase 002.
- `CORE-PHASE-002` is completed through `1cc3d5e6d7227a147b2096c8b85253b083b57a2a` and signed tag `3.0.4-phase-002`. Its sole upstream phase dependency was Phase 001. `EXT-001` was a required public artifact input, not a phase dependency. Its one-time optional-adapter API-resolution cache is scoped only to the exact installed artifact identity and is independent of the Phase 001 context snapshot.
- `CORE-PHASE-003` is active on `envy/3.0.4-phase-003`, whose observed ancestry begins from the verified Phase 002 `origin/master` merge. It owns the corrected Easy Builder recipe serialization, persisted reload, denied-and-eligible runtime proof, inventory insertion feature, and corrected packaged candidate JAR. A source commit alone does not complete a task whose blueprint also requires browser, server, transaction, recovery, or artifact evidence.
- `CORE-PHASE-004` through `CORE-PHASE-006` remain pending and advance contiguously only after the immediately preceding phase is merged into the current `origin/master` and its signed phase tag verifies.
...
Phase dependencies, preserved evidence inputs, external prerequisites, and caches are distinct relationships. The Phase 000 packet is preserved evidence for later phases, not an extra edge that bypasses the contiguous phase chain. `EXT-001`, `EXT-002`, and `EXT-003` gate only the tasks and outcomes named in §10. A phase-local runtime or resolution cache is rebuilt or invalidated by its own lifecycle and never becomes proof that another phase started from the correct branch.
...
The master owns the global order and concise phase catalog. The catalog is frozen as the complete contiguous sequence `CORE-PHASE-000` through `CORE-PHASE-006`; no root-level, skipped, parallel, or implicit phase exists. Every execution blueprint is linked below and registered in the deterministic plan manifest.

The phase-dependency column names only the immediate predecessor that controls branch sequencing. Preserved evidence packets are inputs carried through that chain, not extra phase dependencies. External prerequisites appear in their own column and gate only the named phase work. At every transition, execution freshly verifies the current `origin/master`, the predecessor merge, and the predecessor signed tag, then creates the next branch from that verified default-branch head or safely reconciles the one already applicable branch. No commit recorded by the saved goal or by this planning audit is a substitute for that live check.

| Phase ID | Observed state | Objective | Owner | Immediate phase dependency | External prerequisite or authorization gate | Canonical requirements | Entry summary | Exit summary | Next transition | Blueprint path |
|---|---|---|---|---|---|---|---|---|---|---|
| CORE-PHASE-000 | completed | Freeze the defect, inventory-interaction seam, and artifact baseline | RepositoryAudit | none | none | CORE-REQ-001 | Resolve the live audit revision from verified `origin/master` independently of saved-goal provenance, then pin the six-issue set and separate SRC-009 feature request | Audit classifies every mandatory report and advertised capability claim and records the current CORE-REQ-012 implementation seams | CORE-PHASE-001 | [phases/plan-phase-000.md](phases/plan-phase-000.md) |
| CORE-PHASE-001 | completed | Repair entity presence performance | EntityPresenceEnforcer | CORE-PHASE-000 | none | CORE-REQ-002 | Verified current `origin/master` contains the merged and signed Phase 000 result and CORE-REQ-001 supplies the hot-path fixture | CORE-REQ-002 correctness, same-tick revision invalidation, and DEC-004 performance evidence pass through signed integration | CORE-PHASE-002 | [phases/plan-phase-001.md](phases/plan-phase-001.md) |
| CORE-PHASE-002 | completed | Restore optional integration behavior | OptionalIntegrations | CORE-PHASE-001 | EXT-001 artifact input | CORE-REQ-003, CORE-REQ-004 | Verified current `origin/master` contains the merged and signed Phase 001 result; EXT-001 is complete and is not a phase-dependency edge | Curios, JEI, and EMI compatibility matrix passes through signed integration | CORE-PHASE-003 | [phases/plan-phase-002.md](phases/plan-phase-002.md) |
| CORE-PHASE-003 | active | Complete editor serialization, inventory insertion gating, client UI, and artifact parity | EasyBuilder | CORE-PHASE-002 | none | CORE-REQ-005, CORE-REQ-006, CORE-REQ-007, CORE-REQ-011, CORE-REQ-012 | Verified current `origin/master` contains the merged and signed Phase 002 result; the Phase 000 evidence inputs remain traceable and do not bypass the chain | Production-bundle, corrected-runtime, candidate-JAR, and signed Phase 003 integration evidence pass for editor controls, recipe serialization, two-selector `item_into_inventory` rules, transaction enforcement, UI, and proven artifact parity | CORE-PHASE-004 | [phases/plan-phase-003.md](phases/plan-phase-003.md) |
| CORE-PHASE-004 | pending | Prove compatibility and regression safety | CompatibilityHarness | CORE-PHASE-003 | none | CORE-REQ-008 | Verified current `origin/master` contains the merged and signed Phase 003 result and all transitive evidence inputs remain valid | Full compatibility, inventory-conservation, automation, multiplayer, and security verification passes | CORE-PHASE-005 | [phases/plan-phase-004.md](phases/plan-phase-004.md) |
| CORE-PHASE-005 | pending | Integrate and validate the release artifact | ReleaseValidation | CORE-PHASE-004 | EXT-002 service prerequisite | CORE-REQ-009 | Verified current `origin/master` contains the merged and signed Phase 004 result, and the corrected shared verifier is merged and pinned | Signed master artifact and release validation evidence pass | CORE-PHASE-006 | [phases/plan-phase-005.md](phases/plan-phase-005.md) |
| CORE-PHASE-006 | pending | Publish the verified patch and close issues | ReleaseBroker | CORE-PHASE-005 | EXT-003 authorization before publication | CORE-REQ-010 | Verified current `origin/master` contains the merged and signed Phase 005 result; read-only preview work may begin before artifact-bound authorization | Signed annotated release tag `v3.0.4` verifies at the exact Phase 005 source commit, EXT-003 authorizes only the exact preview, both platforms verify, and the six-issue completion endpoint and Definition of Done pass with no known mandatory phase-owned defect remaining | final completion | [phases/plan-phase-006.md](phases/plan-phase-006.md) |
...
| CORE-REQ-002 | Snapshot construction, reuse, and same-tick revision-invalidation tests | Two player server scenario | Profiled entity tracking and p95 MSPT fixture | Player, tick, and authoritative-state-revision tuple isolation | Spark or equivalent profile capture |
...
- The final JAR, SHA-256, SHA-512, SBOM, source manifest, signed integration, signed annotated release tag `v3.0.4`, and attestation evidence identify the same release artifact
...
Active phase: CORE-PHASE-003
Next executable action: Re-fetch and verify the current `origin/master`, confirm it contains the merged and signed CORE-PHASE-002 result, and resume the applicable `envy/3.0.4-phase-003` branch only after its ancestry passes. Complete the first unfinished Phase 003 evidence gate in task order. The observed recipe-serialization source commit does not by itself complete P003-TASK-007, whose production-bundle, persistence, reload, denied-and-eligible runtime, and rollback proof must pass before P003-TASK-008 through P003-TASK-015 can be accepted. This is current plan state, not a rewrite of immutable `docs/plan/goal.md` or a permanent commit pin.
Known failing checks: GitHub Actions run 31453460136 job 93662356066 fails shared attestation verification because incompatible GitHub CLI signer flags are combined. The completed Phase 000 audit recorded a noncanonical generic visual recipe rule, and CORE-PHASE-003 must still prove the corrected canonical recipe workflows and required runtime and recovery behavior.
